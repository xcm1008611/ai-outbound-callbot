    ...
    private SpeechTranscriberListener getTranscriberListener() {
        return new SpeechTranscriberListener() {
            ...
            public void onTranscriptionResultChange(SpeechTranscriberResponse response) {
                log.info("[阿里云识别] 实时：{}", response.getTransSentenceText());
                if (Fc.isNotBlank(response.getTransSentenceText())) {
                    if (dataReference.get() != null) {
                        boolean is = dataReference.get().apply(response.getTransSentenceText());
                        if (is) {
            ...
            public void onSentenceBegin(SpeechTranscriberResponse response) {
                if (dataReference.get() == null || !aliProperty.getAsrOption().isEnableIntermediateResult()) {
                    statusReference.set(Status.RUN);
                    lastTimeReference.set(System.currentTimeMillis());
                }
            }
            ...
            public void onSentenceEnd(SpeechTranscriberResponse response) {
                log.info("[阿里云识别] 结束：{}", response.getTransSentenceText());
...
//                        if (dataReference.get() != null){
//                            // 持续返回结果
//                            dataReference.get().apply(MESSAGE.toString());
//                            MESSAGE.setLength(0);
//                        } else {
                            // 检测到识别出来的结构立马停止识别
                            transcriber.stop();
