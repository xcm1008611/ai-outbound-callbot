...
public class AgiSupportImpl extends AgiSupport {
    ...
    public AgiSupportImpl(AgiChannel channel, AgiRequest request, Thread thread) {
        ...
        processService = SpringUtil.getBean(IProcessService.class);
    ...
    private UserIntent.Node.StartNode getStartNode() {
        ...
        if (Fc.isNotBlank(routeId)){
            List<? extends UserIntent.Node> flowObj = SpringUtil.getBean(IProcessService.class).findCompleteFlow(Fc.toLong(routeId));
    ...
    protected void realtimeData(Boolean isBot, String filePath, String content) {
        ...
        CallMessageTypeEnum typeEnum = CallMessageTypeEnum.of(isBot);

        // 传递实时会话
        try {
            processService.sendAction(ActionDTO.builder()
                    .type(typeEnum.toValue())
                    .callId(getUniqueId())
                    .content(content)
        ...
        // 保存实时会话
        processService.realtimeMessage(getUniqueId(), content, filePath, typeEnum);
