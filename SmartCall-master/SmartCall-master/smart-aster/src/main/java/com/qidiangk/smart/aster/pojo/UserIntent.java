...
public class UserIntent implements Serializable {
    ...
    @JsonSubTypes({
            ...
            @JsonSubTypes.Type(value = Node.SmsNode.class, name = SmsGranter.GRANT_TYPE),
    })
    ...
    public abstract static class Node implements Serializable {
        ...
        public static class AnswerNode extends Node {
            ...
            @Schema(description = "静默提示")
            private Boolean silencePrompt = false;
            @Schema(description = "静默时长(秒)")
            private Long silenceDuration;
            @Schema(description = "静默提示语")
            private String silenceSay;

            @Schema(description = "不允许打断的话语")
            private List<NoInterruptWord> noInterrupt;

            @Data
            public static class NoInterruptWord {

                @NotNull(message = "ID")
                @Schema(description = "词组ID")
                private Long id;

                @NotEmpty(message = "不允许打断的话语 不可为空")
                @Schema(description = "不允许打断的话语")
                private List<String> word;

                @NotNull(message = "是否精确匹配 不可为空")
        ...
        @Data
        public static class SmsNode extends Node {

            @NotBlank(message = "短信编码 不可为空")
            @Schema(description = "短信编码")
            private String smsCode;

            @Schema(description = "手机号码 支持SPEL表达式")
            @NotBlank(message = "手机号码 不可为空")
            @SpEL(message = "手机号码 不是标准的SPEL表达式")
            private String phone;

            @Schema(description = "参数, 值支持SPEL表达式")
