...
//            nodeContext.getSupport().getVariable("QUEUESTATUS")// 转接状态 如果接听了这个值是CONTINUE
        ...
        if (isSuccess) {
            ...
            // 保存会话记录
            processService.realtimeMessage(nodeContext.getSupport().getUniqueId(), "转接人工：成功", transferFile, CallMessageTypeEnum.TRANSFER);
            ...
            // 保存会话记录
            processService.realtimeMessage(nodeContext.getSupport().getUniqueId(), "转接人工：失败", null, CallMessageTypeEnum.TRANSFER);
