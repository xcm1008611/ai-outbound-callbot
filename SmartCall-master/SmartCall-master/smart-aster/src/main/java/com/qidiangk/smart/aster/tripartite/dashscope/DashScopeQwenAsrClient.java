...
public class DashScopeQwenAsrClient implements AsrClient {
    ...
    private static final class Session {
        ...
        final AtomicReference<Function<String, Boolean>> dataReference = new AtomicReference<>(null);
    }
            ...
            public void onEvent(JsonObject message) {
                ...
                switch (type) {
                    case "input_audio_buffer.speech_started":
                        // 会出现误判情况，所以通过识别到具体文字时候再设置状态
//                        currentSession.statusReference.set(Status.RUN);
                        log.debug("[DashScope-ASR] 检测到语音开始===={}", message.toString());
                        break;
                    case "conversation.item.input_audio_transcription.text":
                        // 当前句子实时预览 = 已确认前缀 text + 临时后缀 stash
                        String text = getString(message, "text");
                        String stash = getString(message, "stash");
//                        log.debug("[DashScope-ASR] 实时预览: {} == {}", text, stash);
                        if (Fc.isNotBlank(stash)) {
                            // 判断是否打断
                            if (currentSession.dataReference.get() != null) {
                                boolean is = currentSession.dataReference.get().apply(stash);
                                if (is) {
                                    log.debug("[DashScope-ASR] 检测到打断: {}", stash);
                                    currentSession.statusReference.set(Status.RUN);
                                }
                            }
                        }
                    ...
                    case "conversation.item.input_audio_transcription.completed":
                        ...
                        if (Fc.isNotBlank(transcript)) {
                            log.debug("[DashScope-ASR] 识别结果: {}", transcript);
                            // 如果和打断配置匹配上了就说明是可以过滤的话语，直接不作为用户提出的问题
                            if (currentSession.dataReference.get() != null) {
                                boolean is = currentSession.dataReference.get().apply(transcript);
                                if (is) {
//                                    log.debug("[DashScope-ASR] 检测到可打断内容: {}", transcript);
...
//                            Boolean is = dataReference.get().apply(currentSession.MESSAGE.toString());
//                            if (is) {
...
//                        }
        ...
        return new AsrResult(future, started, currentSession.dataReference, noDataReference);
    }
