...
public class DashScopeFunAsrClient implements AsrClient {
    ...
    private static final class Session {
        ...
        /** 已匹配成功的词语集合，匹配后永久跳过，不再重复执行 apply */
        final Set<String> matchedWords = ConcurrentHashMap.newKeySet();
        ...
        final ResultCallback<RecognitionResult> callback = new ResultCallback<RecognitionResult>() {
            ...
            public void onEvent(RecognitionResult result) {
                ...
                if (dataReference.get() != null) {
                    ...
                    if (Fc.isNotEmpty(sentence.getWords())) {
                        sentence.getWords().forEach(word -> {
                            log.debug("[DashScope-FunASR] 识别词: {}", word.getText());
                            if (Fc.isNotBlank(word.getText()) && !matchedWords.contains(word.getText())) {
                                boolean is = dataReference.get().apply(word.getText());
                                // 匹配成功，加入集合，后续不再重复执行 apply
                                matchedWords.add(word.getText());
                                if (is) {
                ...
                if (result.isSentenceEnd()) {
                    ...
                    if (Fc.isNotBlank(text) && (dataReference.get() == null || dataReference.get().apply(text))) {
...
//                            dataReference.get().apply(currentSession.message.toString());
