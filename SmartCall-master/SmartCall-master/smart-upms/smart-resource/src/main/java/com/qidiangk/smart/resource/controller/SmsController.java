...
import com.qidiangk.smart.resource.pojo.SmsSelectVO;
import com.qidiangk.smart.resource.pojo.SmsSendVO;
...
public class SmsController extends BaseController {
    ...
    @Function(value = "短信下拉",menus = {
        @Menu(client = "admin",menuCode = "CALL_ROUTER",btnCode = "CALL_ROUTER_CONFIG",code = "CALL_ROUTER_SMS_SELECT", type = Menu.TYPE.INTERFACE)
    })
    @Operation(summary = "短信下拉")
    @GetMapping(value = "/select", produces = APPLICATION_JSON_VALUE)
    public R<List<SmsSelectVO>> select(){
        return R.data(resourceSmsService.listAs(Wrappers.getQueryWrapper()
                .select(ResourceSms::getCode, ResourceSms::getName, ResourceSms::getParameters), SmsSelectVO.class));
    }
